...
def generate_problem(name, num_blocks):
    ...
    try:
        goal_block = random.choice([b for b in blocks if clear(b) not in init])
    except IndexError:
        goal_block = random.choice(blocks)
    problem = Problem(name, domain=domain, objects=blocks, init=init, goal=clear(goal_block))
    return problem
