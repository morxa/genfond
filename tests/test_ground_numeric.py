...
def test_ground_childsnack(childsnack):
    ...
    assert {action.name
            for action in ground_actions} == {
                'make_sandwich_no_gluten', 'make_sandwich', 'put_on_tray', 'serve_sandwich_no_gluten',
                'serve_sandwich', 'move_tray'
            }
    ...
    assert serve_sandwich.precondition == And(
        at(tray1, kitchen), LesserEqualThan(NumericValue(1), NumericFunction('ontray', tray1, is_gluten_free)),
