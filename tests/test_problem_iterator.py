...
def test_one_shot_problem_iterator_returns_single_max_complexity_item():
    ...
    assert iteration["max_prune_cost"] == MAX_COST
    assert iteration["selected_states"] == {}
    with pytest.raises(StopIteration):
        next(iterator)


def test_one_shot_problem_iterator_initializes_selected_states_for_all_problems():
    a, b = constants("a b")
    problems = [
        DummyProblem("p1", frozenset({Predicate("at", a)})),
        DummyProblem("p2", frozenset({Predicate("at", b)})),
    ...
    assert set(iteration["selected_states"].keys()) == {"p1", "p2"}
    assert iteration["selected_states"]["p1"] == {problems[0].init}
    assert iteration["selected_states"]["p2"] == {problems[1].init}
