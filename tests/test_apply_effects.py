from genfond.state_space_generator import apply_action_effects
...
def test_apply_effects_simple_blocks(simple_blocks):
    ...
    states = apply_action_effects(problem.init,
                                  get_action(ground_actions, 'pick', (a, b)))
    assert states == frozenset([frozenset([holding(a)])])
    states = apply_action_effects(next(iter(states)),
                                  get_action(ground_actions, 'put', (a, b)))
...
def test_apply_effects_fond_blocks(fond_blocks):
    ...
    states = apply_action_effects(
        problem.init, get_action(ground_actions, 'puton', (c, b, a)))
    assert len(states) == 2
    assert states == {
        frozenset(blocks | {
            on(a, table),
