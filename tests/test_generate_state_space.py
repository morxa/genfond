...
def test_generate_state_space_simple_blocks(simple_blocks):
    ...
    assert set(pick_ab_node.children.keys()) == {put_ab, put_aa}
...
def test_generate_state_space_fond_blocks(fond_blocks):
    ...
    assert set(
        state_space.root.children.keys()) == {cta_puton, cba_puton, bct_puton}
    assert len(state_space.root.children[cta_puton]) == 1
    assert {node.state
            for node in state_space.root.children[cta_puton]
            } == apply_action_effects(problem.init, cta_puton)
    assert len(state_space.root.children[cba_puton]) == 2
    # cta_puton and cba_puton lead to a common node
    assert len(state_space.root.children[cta_puton]
               & state_space.root.children[cba_puton]) == 1
    assert {node.state
            for node in state_space.root.children[cba_puton]
            } == apply_action_effects(problem.init, cba_puton)
    assert len(state_space.root.children[bct_puton]) == 2
    assert {node.state
            for node in state_space.root.children[bct_puton]
            } == apply_action_effects(problem.init, bct_puton)
    ...
    assert set(cta_puton_node.children.keys()) == {
        abt_puton, act_puton, bat_puton, bct_puton, cat_puton, cbt_puton
    }
    assert cta_puton_node.children[cat_puton] == {
        state_space.root, cta_puton_node
    }
