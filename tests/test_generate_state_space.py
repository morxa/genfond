...
def test_generate_state_space_fond_blocks(fond_blocks):
    ...
    assert {node.state
            for node in state_space.root.children[cta_puton]} == apply_action_effects(problem.init, cta_puton)
    ...
    assert {node.state
            for node in state_space.root.children[cba_puton]} == apply_action_effects(problem.init, cba_puton)
    assert len(state_space.root.children[bct_puton]) == 2
    assert {node.state
            for node in state_space.root.children[bct_puton]} == apply_action_effects(problem.init, bct_puton)
