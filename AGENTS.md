...
A problem with **no** example plan is the degenerate case: `StateSpaceGraph` has nothing to restrict itself to and expands the whole instance, and the problems the planner cannot solve are exactly the hard ones (on sokoban that expansion never returned). `defer_planless_problems` (default true, inert without `use_example_plans`) therefore keeps such a problem out of the *training* set while leaving it in the evaluation set, retries it `planless_retries` times against a reseeded planner stream, and stops the run with `failureReason=no_example_plans` if no problem has a plan at all. `max_states_per_problem` (default null) is the safety net for a plan-restricted expansion that is still too large: it aborts the expansion and defers that problem for good. The expansion loop also polls `stop_requested()` every `STOP_CHECK_INTERVAL` nodes, which is the only way a SIGTERM reaches it. See `docs/defer-planless-results.md`.
