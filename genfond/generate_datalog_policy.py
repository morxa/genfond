...
def generate_datalog_policy(solution):
    ...
    for instance, state, action, param, _, _, _, feature in solution.get('aug_fdist', []):
        aug_dist_features.setdefault((instance, state, action), []).append((feature, param))
    diff_conds = dict()
